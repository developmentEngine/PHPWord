...
use Devengine\PhpWord\Element\Row;
...
class Html
{
     ...
     *
     * @param \Devengine\PhpWord\Element\AbstractContainer $element Where the parts need to be added
     ...
     * @param \Devengine\PhpWord\Element\AbstractContainer $element object to add an element corresponding with the node
    ...
    protected static function parseNode($node, $element, $styles = array(), $data = array())
    {
        ...
        if (isset($nodes[$node->nodeName])) {
            ...
            $method = "parse{$method}";
            $newElement = call_user_func_array(array('Devengine\PhpWord\Shared\Html', $method), $arguments);
     ...
     *
     * @param \DOMNode $node
     * @param \Devengine\PhpWord\Element\AbstractContainer $element
     * @param array $styles
     * @param array $data
     */
     ...
     *
     * @param \DOMNode $node
     * @param \Devengine\PhpWord\Element\Table $element
     * @param array &$styles
     * @return Row $element
     */
     ...
     * @param \Devengine\PhpWord\Element\Table $element
     * @param array &$styles
     * @return \Devengine\PhpWord\Element\Cell|\Devengine\PhpWord\Element\TextRun $element
     */
            ...
            switch (trim($cKey)) {
                ...
                case 'line-height':
                    ...
                    if (preg_match('/([0-9]+\.?[0-9]*[a-z]+)/', $cValue, $matches)) {
                        ...
                        $spacingLineRule = \Devengine\PhpWord\SimpleType\LineSpacingRule::EXACT;
                        $spacing = Converter::cssToTwip($matches[1]);
                    } elseif (preg_match('/([0-9]+)%/', $cValue, $matches)) {
                        //matches percentages
                        $spacingLineRule = \Devengine\PhpWord\SimpleType\LineSpacingRule::AUTO;
                ...
                case 'width':
                    if (preg_match('/([0-9]+[a-z]+)/', $cValue, $matches)) {
                        ...
                        $styles['unit'] = \Devengine\PhpWord\SimpleType\TblWidth::TWIP;
                    } elseif (preg_match('/([0-9]+)%/', $cValue, $matches)) {
                        $styles['width'] = $matches[1] * 50;
                        $styles['unit'] = \Devengine\PhpWord\SimpleType\TblWidth::PERCENT;
                    } elseif (preg_match('/([0-9]+)/', $cValue, $matches)) {
                        $styles['width'] = $matches[1];
                        $styles['unit'] = \Devengine\PhpWord\SimpleType\TblWidth::AUTO;
                    }
                    break;
                case 'border':
     ...
     * @param \Devengine\PhpWord\Element\AbstractContainer $element
     *
     * @return \Devengine\PhpWord\Element\Image
     **/
        ...
        foreach ($node->attributes as $attribute) {
            switch ($attribute->name) {
                ...
                case 'width':
                    ...
                    $style['width'] = $width;
                    $style['unit'] = \Devengine\PhpWord\Style\Image::UNIT_PX;
                    break;
                case 'height':
                    ...
                    foreach ($styleattr as $attr) {
                        if (strpos($attr, ':')) {
                            ...
                            switch ($k) {
                                case 'float':
                                    if (trim($v) == 'right') {
                                        $style['hPos'] = \Devengine\PhpWord\Style\Image::POS_RIGHT;
                                        $style['hPosRelTo'] = \Devengine\PhpWord\Style\Image::POS_RELTO_PAGE;
                                        $style['pos'] = \Devengine\PhpWord\Style\Image::POS_RELATIVE;
                                        $style['wrap'] = \Devengine\PhpWord\Style\Image::WRAP_TIGHT;
                                        $style['overlap'] = true;
                                    }
                                    if (trim($v) == 'left') {
                                        $style['hPos'] = \Devengine\PhpWord\Style\Image::POS_LEFT;
                                        $style['hPosRelTo'] = \Devengine\PhpWord\Style\Image::POS_RELTO_PAGE;
                                        $style['pos'] = \Devengine\PhpWord\Style\Image::POS_RELATIVE;
                                        $style['wrap'] = \Devengine\PhpWord\Style\Image::WRAP_TIGHT;
                                        $style['overlap'] = true;
    ...
    /**
     * Transforms a HTML/CSS alignment into a \Devengine\PhpWord\SimpleType\Jc
     *
